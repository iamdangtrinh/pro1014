...
if (isset($_GET['act'])) {
    switch ($_GET['act']) {
        ...
        case 'update_info':
            include_once 'model/m_user.php';
            if(isset($_POST['bt_update_info']) && $_POST['btn_update_info']){
                echo 'Vô';
                $HoTen = $_POST['HoTen'];
                $SoDienThoai = $_POST['SoDienThoai'];
                $DiaChi = $_POST['DiaChi'];
                $user_update_info = user_edit($MaTK,$SoDienThoai, $Email, $HoTen, $MatKhau, $DiaChi,$VaiTro);
                if (empty($HoTen) || empty($SoDienThoai) || empty($DiaChi)){
                    $_SESSION['ThongBao']['Loi'] = 'Vui lòng điền đủ thông tin!';
                } else if($HoTen = $_POST['HoTen']){
                    $_SESSION['ThongBao']['Loi'] = 'Vui lòng nhập tên khác!';
                } else if($SoDienThoai = $_POST['SoDienThoai']){
                    $_SESSION['ThongBao']['Loi'] = 'Vui lòng nhập số điện thoại khác!';
                } else if($DiaChi = $_POST['DiaChi']){
            ...
            header("location: '.$base_url.' user/dashboard#account");
            $view_name = 'user_update_info';
            break;
        case 'forgotPassword':
